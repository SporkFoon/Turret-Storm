"""GameMap class - handles the tile-based game map rendering."""
...
class GameMap:
    """Renders the game map grid with path tiles and grass tiles.
    
    Attributes:
        grid: 2D array of tile types
        path_tiles: Set of (col, row) that are path
    """
    ...
    def grid_to_pixel_center(self, col, row):
        # convert grid pos to pixel center
        x = GRID_OFFSET_X + col * TILE_SIZE + TILE_SIZE // 2
        y = GRID_OFFSET_Y + row * TILE_SIZE + TILE_SIZE // 2
        return x, y

    def draw(self, surface):
        # draw the complete map using sprite 
        import assets  # optimization (local import avoids circular import at module load)
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if sprite is not None:
                    ...
                    # defensive fallback should the cache be empty
                    rect = pygame.Rect(x, y, TILE_SIZE, TILE_SIZE)
                    pygame.draw.rect(surface, GREEN, rect)

        # ssubtle grid lines keep the "tile-based" feel readable
        for row in range(self.rows + 1):
            y = GRID_OFFSET_Y + row * TILE_SIZE
            pygame.draw.line(surface, (0, 0, 0, 30), (GRID_OFFSET_X, y),
                             (GRID_OFFSET_X + self.cols * TILE_SIZE, y), 1)
        for col in range(self.cols + 1):
            x = GRID_OFFSET_X + col * TILE_SIZE
            pygame.draw.line(surface, (0, 0, 0, 30), (x, GRID_OFFSET_Y),
                             (x, GRID_OFFSET_Y + self.rows * TILE_SIZE), 1)
