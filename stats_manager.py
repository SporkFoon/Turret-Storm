...
class StatsManager:
    ...
    def record_wave_end(self, wave_num, enemies_killed, enemies_leaked, gold_earned):
        ...
        self.wave_stats.append({'wave': wave_num, 'enemies_killed': enemies_killed, 'enemies_leaked': enemies_leaked, 'gold_earned': gold_earned, 'wave_time': round(wave_time, 2), 'game_time': round(self.game_time, 2)})
        self.wave_completion_times.append({'wave': wave_num, 'completion_time': round(wave_time, 2), 'kill_count': enemies_killed, 'leak_count': enemies_leaked})

    def record_tower_placed(self, tower_type, col, row, wave_num, cost):
        self.total_towers_placed += 1
        self.tower_placements.append({'id': self.total_towers_placed, 'tower_type': tower_type, 'col': col, 'row': row, 'wave': wave_num, 'cost': cost, 'game_time': round(self.game_time, 2)})

    def record_enemy_kill(self, enemy_type, tower_type, damage_dealt, wave_num):
        self.total_kills += 1
        self.enemy_kills.append({'kill_id': self.total_kills, 'enemy_type': enemy_type, 'killed_by': tower_type, 'damage_dealt': damage_dealt, 'wave': wave_num, 'game_time': round(self.game_time, 2)})
    ...
    def record_gold_change(self, amount, reason, balance, wave_num):
        ...
        self.gold_transactions.append({'amount': amount, 'reason': reason, 'balance': balance, 'wave': wave_num, 'game_time': round(self.game_time, 2)})

    def record_damage(self, tower_type, damage, enemy_type, wave_num):
        self.total_damage_dealt += damage
        self.damage_events.append({'tower_type': tower_type, 'damage': damage, 'enemy_type': enemy_type, 'wave': wave_num, 'game_time': round(self.game_time, 2)})

    def record_action(self, action_type, details=''):
        self.player_actions.append({'action': action_type, 'details': details, 'game_time': round(self.game_time, 2)})

    def save_to_csv(self):
        output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stats_data')
        os.makedirs(output_dir, exist_ok=True)
        datasets = {'wave_stats.csv': self.wave_stats, 'tower_placements.csv': self.tower_placements, 'enemy_kills.csv': self.enemy_kills, 'gold_transactions.csv': self.gold_transactions, 'damage_events.csv': self.damage_events, 'wave_completion_times.csv': self.wave_completion_times, 'player_actions.csv': self.player_actions}
    ...
    def _draw_overview_page(self, surface, font_header, font_text):
        ...
        y += 35
        stats = [f'Total Waves Completed: {len(self.wave_stats)}', f'Total Enemies Killed: {self.total_kills}', f'Total Damage Dealt: {self.total_damage_dealt}', f'Total Gold Earned: {self.total_gold_earned}', f'Total Gold Spent: {self.total_gold_spent}', f'Total Towers Placed: {self.total_towers_placed}', f'Total Game Time: {self.game_time:.1f}s', f'Total Player Actions: {len(self.player_actions)}']
        for s in stats:
        ...
        y += 35
        records = [('Wave Stats', len(self.wave_stats)), ('Tower Placements', len(self.tower_placements)), ('Enemy Kills', len(self.enemy_kills)), ('Gold Transactions', len(self.gold_transactions)), ('Damage Events', len(self.damage_events)), ('Wave Completion Times', len(self.wave_completion_times)), ('Player Actions', len(self.player_actions))]
        total_records = 0
